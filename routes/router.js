...
router.get("/getproductsone/:id", async (req,res) => {
    try{
        ...
        res.status(400).send('Error come from get individual data');
...
router.post('/register',async (req,res) => {
    ...
    const {fname,email,mobile,password,confirmpassword} = req.body;

    if(!fname || !email || !mobile || !password || !confirmpassword) {
...
router.post('/login',async(req,res) => {
    ...
    if(!email || !password) {
        res.status(401).json({error: "fill the all data"})
    };
